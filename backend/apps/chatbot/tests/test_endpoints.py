...
from backend.apps.chatbot.models import *
...
@pytest.fixture
def access_token(
    client: APIClient, mock_email: str, mock_password: str, auth_user: Account
) -> str:
...
def test_token_view_authorized(
    client: APIClient, mock_email: str, mock_password: str
):
...
@pytest.mark.django_db
def test_token_view_unauthorized(
    client: APIClient, mock_email: str, mock_password: str
):
    _ = Account.objects.create(
        email=mock_email,
