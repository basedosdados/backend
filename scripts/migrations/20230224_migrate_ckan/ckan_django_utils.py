...
from data.enums.entity import EntityDateTimeEnum, EntityEnum
...
def fetch_package_data(package_name):
    url = f"https://basedosdados.org/api/3/action/package_show?name_or_id={package_name}"
    return requests.get(url, verify=False).json()["result"]
...
def request_bd_packages(file_path):
    ...
    new_package_names = [name for name in package_list if name not in existing_package_names]
...
def build_areas_from_json():
    with open("./app/schemas/repository/bd_spatial_coverage_tree.json") as f:
        area = json.load(f)
...
def class_to_dict(class_obj):
    return {name: getattr(class_obj, name) for name in dir(class_obj) if not name.startswith("__")}
...
class Migration:
    def __init__(self, mode):
        ...
        self.df_area = pd.read_csv("./utils/migration/data/enums/spatial_coverage_tree.csv")
    ...
    def create_update(
        ...
        response, node_id = self.get_id(query_class=query_class, query_parameters=query_parameters)
        time.sleep(0.1)
        ...
        if "data" not in response or response is None:
            self.get_mutation_error(mutation_class, query, mutation_parameters, response)
    ...
    def create_tags(self, objs):
        ...
        for obj in objs:
            ...
            tag_name = "desconhecida" if tag_name is None else tag_name.replace(" ", "_")
    ...
    def create_entity(self, obj=None):
        if obj is None or obj.get("entity") is None:
            ...
            entity_dict = {
                key: value for entity in EntityEnum for key, value in class_to_dict(entity).items()
            }
    ...
    def _create_update_frequency_for_entity(self, entity_id, resource_id, update_frequency_number):
        mutation_parameters = {
            "entity": entity_id,
    ...
    def create_bq_type(self, obj):
        r, id = self.create_update(
            ...
            query_parameters={"$name: String": obj.upper() if type(obj) == str else "desconhecida"},
        )
    ...
    def create_temporal_coverage(self, resource, coverage_id, column=None):
        ...
        if column is not None:
            temporal_temporal_coverages = (
                [None]
                if column.get("temporal_coverage") == [] or column.get("temporal_coverage") is None
    ...
    def _create_column(self, column, table_id, resource):
        ...
        mutation_parameters = {
            "table": table_id,
            "bigqueryType": self.create_bq_type(column.get("bigquery_type", "desconhecida")),
            "name": column_name,
        ...
        if resource.get("observation_level") is not None:
            obs_level = self.get_obs_level(resource.get("observation_level"), column_name)
        else:
            obs_level = None
    ...
    def create_org(self, org_dict):
        ...
        if org_dict is None:
            ...
            org_description = (
                org_dict.get("description", None) or org_dict.get("notes", None) or "desconhecida"
            )
            org_id = org_dict.get("name") or org_dict.get("organization_id")
            org_slug = (
                "desconhecida"
                if org_id is None
                else org_id.replace("-", "_").replace(".", "_").lower().replace(" ", "_")
            )
            org_slug = unidecode(org_slug)

            org_slug_parts = org_slug.split("_", 1)
            df = self.df_area
            df["possible_areas"] = df["id"].apply(lambda x: x.replace(".", "_").split("_", 1)[-1])
            ...
            if (
                ...
                org_url = re.search("(?P<url>https?://[^\s]+)", org_description).group("url")
